...
pub fn get_constant_pool(constant_pool_count:u16, reader: &mut Cursor<Vec<u8>>) -> Vec<ConstantPoolTags>{
    {
        ...
        for i in 0..constant_pool_count-1{
            ...
            let info:ConstantPoolTags = match tag {
                ...
                9 | 10 | 11 => { // all have the same data, just diferent names
                    let class_index:u16 = reader.read_be::<u16>().unwrap();
                    let name_and_type_index:u16 = reader.read_be::<u16>().unwrap();
                    match tag { //recalculate it into the right enum type
                        9 =>  ConstantPoolTags::Fieldref(class_index, name_and_type_index),
                        10 => ConstantPoolTags::Methodref(class_index, name_and_type_index),
                        11 => ConstantPoolTags::InterfaceMethodref(class_index, name_and_type_index),
                        _ => {panic!("Somehow the tag changed in 3 insturctions, uhhhh, maybe a cosmic ray did a bitflip?")}
